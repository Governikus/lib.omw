...
public class TLV {
     ...
     * @param ofs     - destination offset
     * @param value   - tag value
     * @param valSize - size of tag value (amount of bytes)
     */
    ...
    /**
     * Builds the <em>BER-TLV</em>-object according to <em>ISO/IEC8825</em> and <em>ASN.1</em>.
     *
     * @param tagID - tagID
     * @param data  - content
     ...
     *
     * @param data - <em>BER-TLV</em> encoded input data
    ...
    public static byte[] get(final byte[] data, final byte[] tag) {
        ...
        try {
            ...
            while (i + tag.length + 1 < data.length) {
                ...
                if (data[i] == tag[0]) {
                    if (tag.length == 1 || ArrayTool.equals(ArrayTool.sub(data, i, tag.length), tag)) {
                        break;
                    }
                }
     ...
     *
     * @param data - BER-TLV encoded string
    ...
    public static String getString(final byte[] data) {
        ...
        switch (data[0]) {
            case STRING_UTF8:
                return new String(TLV.get(data, data[0]), StandardCharsets.UTF_8);
            ...
            case STRING_ISO646:
            case STRING_GENERAL:
                return new String(TLV.get(data, data[0]), StandardCharsets.US_ASCII);

            case STRING_BMP:
                return new String(TLV.get(data, data[0]), StandardCharsets.UTF_16BE);
        }
    ...
    public static List<byte[]> getM(final byte[] data, final byte[] tag) {
        ...
        try {
            ...
            list.add(ArrayTool.sub(data, i + tag.length + sizelen, datalen));

            i += tag.length + sizelen + datalen;
            if (i < data.length) {
                byte[] remainder = ArrayTool.sub(data, i, data.length - i);
     ...
     *
     * @param b1      - first source array
     * @param offset1 - offset in first source array
     * @param length1 - length for first source array
     * @param b2      - second source array
     * @param offset2 - offset in second source array
     * @param length2 - length for second source array
    ...
    public static final byte[] concat(final byte[]... in) {
